...
const Opened = () => {
  ...
  const getInfo = async () => {
    ...
    try{
        const buyerInfo = await presalecontract.buyers("0x0de028201d03CB972758353B75a9db1FbA6D912f");
        setBuyerInfo(
          [
            { id: "Invested", val: ethers.utils.formatUnits(buyerInfo.base, 18).toString() + " " + chainSuffix },
            { id: "AXL Amount", val: ethers.utils.formatUnits(buyerInfo.sale, 18).toString() + " " + tokeninfoarr.symbol},
          ]
        )
    }
    catch (error)
